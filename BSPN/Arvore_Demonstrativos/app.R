...
m= estrutura_pcasp%>%data.frame%>%mutate(NEWCOL=NA)%>%distinct
...
server <- function(input, output) {
  ...
  observeEvent(input$Hierarchy,{
    output$d3 <- renderD3tree({
      ...
      d3tree(data = list(root = df2tree(struct = p,rootname = 'BP'),layout = 'collapse'), width = '200%', activeReturn = c('name','value','depth','id'),height = 18)
    })
  })
